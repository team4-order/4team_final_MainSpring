...
import main.spring.login.demo2.service.CStorageService1;
...
@RestController
@RequestMapping("/api/cstorage1")
public class CStorageController1 {

    private final CStorageService1 cStorageService1 ;
    ...
    public CStorage addOrUpdateCStorage(@RequestBody CStorage cStorage) {
        return cStorageService1.addOrUpdateCStorage(cStorage.getCustomerCode(), cStorage.getStorageCode());
    }
    ...
    public ResponseEntity<?> deleteCStorage(@PathVariable String customerCode, @PathVariable String storageCode) {
        boolean isDeleted = cStorageService1.deleteCStorage(customerCode, storageCode);
    ...
    public ResponseEntity<List<CStorageDTO>> getAllCStoragesWithContactName() {
        ...
        return ResponseEntity.ok(cStorageService1.findAllCStoragesWithContactName());
    }
    ...
    public ResponseEntity<?> updateCStorageList(@PathVariable String customerCode, @RequestBody List<String> storageCodes) {
        try {
            ...
            cStorageService1.deleteAllCStoragesByCustomerCode(customerCode);

            // 새로운 창고 코드 목록으로 업데이트
            for (String storageCode : storageCodes) {
                cStorageService1.addOrUpdateCStorage(customerCode, storageCode);
            }
            return ResponseEntity.ok().build();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<?> assignStoragesToCustomer(@PathVariable String customerCode, @RequestBody List<String> storageCodes) {
        try {
            ...
            cStorageService1.deleteAllStoragesForCustomer(customerCode);

            // 다음, 제공된 창고 코드 리스트를 해당 거래처에 할당
            storageCodes.forEach(storageCode -> cStorageService1.addOrUpdateCStorage(customerCode, storageCode));
